...
static bool wsrep_tables_accessible_when_detached(const TABLE_LIST *tables) {
  ...
  for (const TABLE_LIST *table = tables; table; table = table->next_global) {
    TABLE_CATEGORY c;
    LEX_STRING db, tn;
    // Using LEX_STRING (vs LEX_CSTRING) as demanded by get_table_category
    lex_string_set(&db, const_cast<char*> (table->db));
    lex_string_set(&tn, const_cast<char*> (table->table_name));
...
static void wsrep_mysql_parse(THD *thd, const char *rawbuf, uint length,
  ...
  do {
    ...
    /* TODO: check_secondary_engine_statement functionality is incomplete.
    Add it once MySQL define semenatics for it. */
